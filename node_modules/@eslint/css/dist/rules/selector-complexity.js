...
export default /** @satisfies {SelectorComplexityRuleDefinition} */ {
    ...
    create(context) {
        ...
        const disallowedPseudoClasses = new Set(disallowPseudoClasses.map(selector => selector.toLowerCase()));
        const disallowedPseudoElements = new Set(disallowPseudoElements.map(selector => selector.toLowerCase()));
        return {
            Selector(node) {
                ...
                if (disallowPseudoClasses.length > 0) {
                    for (const selectorNode of pseudoClassSelectors) {
                        if (disallowedPseudoClasses.has(selectorNode.name.toLowerCase())) {
                            context.report({
                ...
                if (disallowPseudoElements.length > 0) {
                    for (const selectorNode of pseudoElementSelectors) {
                        if (disallowedPseudoElements.has(selectorNode.name.toLowerCase())) {
                            context.report({
                                loc: selectorNode.loc,
                                messageId: "disallowedSelectors",
                                data: {
