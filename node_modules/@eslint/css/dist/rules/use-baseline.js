...
// Imports
//-----------------------------------------------------------------------------
import { BASELINE_HIGH, BASELINE_LOW, properties, propertyValues, atRules, mediaConditions, functions, units, selectors, globalKeywords, } from "../data/baseline-data.js";
        ...
        schema: [
            {
                ...
                properties: {
                    ...
                    allowPropertyValues: {
                        type: "object",
                        properties: Object.fromEntries(Array.from(properties.keys()).map(prop => {
                ...
                for (let i = 0; i < node.children.length; i++) {
                    ...
                    if (conditionChild.type === "SupportsDeclaration") {
                        ...
                        declaration.value.children.forEach(child => {
                            if (child.type === "Identifier") {
                                supportedProperty.addIdentifier(child.name.toLowerCase());
                                return;
                            }
                            if (child.type === "Dimension") {
                                supportedProperty.addUnit(child.unit.toLowerCase());
                                return;
                            }
                            if (child.type === "Function") {
                                supportedProperty.addFunction(child.name.toLowerCase());
                            }
                        });
                        continue;
                    }
                    if (conditionChild.type === "FeatureFunction" &&
                        ...
                        for (const selectorChild of conditionChild.value
                            .children) {
                            supportsRule.addSelector(selectorChild.type === "NestingSelector"
                                ? "nesting"
            ...
            "Rule > Block > Declaration"(node) {
                ...
                for (const child of node.value.children) {
                    if (child.type === "Identifier") {
                        ...
                        if (!supportsRules.hasPropertyIdentifier(property, child.name.toLowerCase())) {
                            checkPropertyValueIdentifier(property, child);
                        }
                        continue;
                    }
                    if (child.type === "Dimension") {
                        if (!supportsRules.hasPropertyUnit(property, child.unit.toLowerCase())) {
                            checkPropertyValueUnit(property, child);
                        }
                        continue;
                    }
                    if (child.type === "Function") {
                        if (!supportsRules.hasPropertyFunction(property, child.name.toLowerCase())) {
